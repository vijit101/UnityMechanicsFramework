...
| 3 | [Stat System with Modifiers](#3-stat-system-with-modifiers) | — | Systems / RPG | — |
...
### 3. Stat System with Modifiers

| | |
|---|---|
| **Author** | — |
| **Namespace** | `GameplayMechanicsUMFOSS.Systems` |
| **Location** | `Runtime/Systems/StatSystem_UMFOSS.cs` |
| **Category** | Systems / RPG / Combat |
| **Demo Scene** | `Samples~/StatSystem/Assets/Scenes/DemoScene.unity` |
| **Video** | — *(record using the Setup Guide)* |

**What it does**

A generic, extensible stat engine for any numeric value — health, damage, speed, defence, crit chance, mana.  Any source (sword, potion, status effect, ability) can apply flat and percentage modifiers that stack in the correct industry-standard order, and remove themselves automatically when their source expires or is unequipped.  The foundation of every RPG, roguelike, and upgrade system.
...
// 1. Add StatSheet_UMFOSS to your player/enemy GameObject via Inspector.

// 2. Apply modifiers from any system:
var sword = GetComponent<SwordItem>();
var damageBonus = new StatModifier_UMFOSS(15f, ModifierType_UMFOSS.Flat, source: sword);
playerStatSheet.AddModifier(StatType_UMFOSS.Damage, damageBonus);

// 3. Read the final value (dirty-flag cached — safe to call every frame):
float damage = playerStatSheet.GetValue(StatType_UMFOSS.Damage);

// 4. Timed modifier — removes itself after 5 seconds, no coroutines:
var debuff = new StatModifier_UMFOSS(-0.3f, ModifierType_UMFOSS.PercentAdd, source: poisonEffect);
playerStatSheet.AddTimedModifier(StatType_UMFOSS.MoveSpeed, debuff, 5f);

// 5. Unequip sword — removes all of its modifiers across every stat in one call:
playerStatSheet.RemoveAllModifiersFromSource(sword);

// 6. Save system — flat dictionary of final values:
Dictionary<StatType_UMFOSS, float> snapshot = playerStatSheet.GetSnapshot();
```

**Highlights**

- **Correct stacking order** — flat modifiers always apply before percentage modifiers, matching every shipped RPG (Diablo, Path of Exile, Elden Ring)
- **Three modifier types** — `Flat` (additive), `PercentAdd` (additive percentage), `PercentMultiply` (multiplicative, for legendary-tier power spikes)
- **Dirty flag cache** — `FinalValue` recalculates only when modifiers change, not every read
- **Source-based removal** — `RemoveAllModifiersFromSource(sword)` strips everything a source applied across all stats in one call — no tracking needed
- **Self-expiring timed modifiers** — `AddTimedModifier` uses `TimerUtility_UMFOSS` internally; no coroutines in `StatSheet`
- **Float drift prevention** — `Math.Round` to 4 decimal places on every calculation
- **EventBus decoupling** — stat changes fire `StatChangedEvent`; UI and HealthSystem react without direct references
- **Save-system ready** — `GetSnapshot()` returns `Dictionary<StatType, float>`, serializable by any save system
